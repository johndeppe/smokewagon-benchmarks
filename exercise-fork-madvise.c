...
void* test_smokewagon(void* tid) {
    ...
    if (result != 0) {
        printf("tid %ld: madvise smokewagonified: %d, errno: %d, strerror: %s\n", (long) tid, result, errno, strerror(errno));
    }

    // fork
    pid_t my_pid = fork();
    printf("tid %ld: pid: %lu: forked.\n", (long) tid, (unsigned long) my_pid);

    // touch second half:
    for (long i = PAGES*PAGE_SIZE/2; i < PAGES*PAGE_SIZE; i++ ) {
        ptr[i] = 'x';
    }
    printf("tid %ld: pid: %lu touched all allocated memory.\n", (long) tid, (unsigned long) my_pid);
    ...
    if (result != 0) {
	    printf("tid %ld: pid: %lu: madvise normalized: %d, errno: %d, strerror: %s\n", (long) tid, (unsigned long) my_pid, result, errno, strerror(errno));
    }
